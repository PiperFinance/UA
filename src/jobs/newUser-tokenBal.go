...
func (u *SyncAddress) tokenBal(c context.Context) error {
	...
	for u.tokenBalRetries > 0 {
		url := conf.Config.PortfolioScannerURL.JoinPath(conf.Config.PSSafeTokenBalUrl)
		q := url.Query()
...
func (u *SyncAddress) saveTokenBal(c context.Context) error {
	if u.tokenBalRetries == 0 {
